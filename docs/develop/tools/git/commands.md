...
!!! tip
    **Git 与 GitHub**。Git 是一款版本管理软件，适用目前绝大多数操作系统；[GitHub](./github.md) 是一个代码托管平台，与 Git 没有任何关系。只不过 Git 可以基于 GitHub 进行分布式管理。因此往往需要结合二者来达到相对良好的 Teamwork 效果。

    **仓库连接协议**。基于 GitHub 等代码托管平台进行分布式开发时，涉及到连接协议的选择问题，主要有 HTTPs 和 SSH 两种选项，具体用那种以及对应的配置方法详见 [GitHub 身份鉴权](./github.md#身份鉴权)，本文不再赘述。

    **命令参数编写规则**。很多工具的命令都会有参数配置选项，而参数意味着不确定性，你可能会在下文中遇到以下几种情况：
...
本文主要从「配置、迭代、回溯、分支、远程」五个方面介绍 Git 的常用命令，完整内容见 [Git](https://git-scm.com/book/zh/v2) 官方文档。
