...
假设你已经有了一台云服务器。为了简化后续文件上传操作，需要建立本地机和云服务器的 [SSH](../operation/ssh.md) 连接。下面将基于 [Git Hooks](https://githooks.com/) 工具，介绍如何将自己的 Hexo 静态网站部署到阿里云服务器上。
...
然后安装并配置 [Nginx](../operation/nginx/index.md)，确保外网也可以访问服务器。
...
同样需要先配置好 [SSH](../operation/ssh.md) 和 [Nginx](../operation/nginx/setup.md)。接下来就两步：
