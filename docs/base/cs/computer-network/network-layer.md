...
由于 IPv4 地址被快速消耗造成短缺加上 IPv6 迁移进程缓慢，亟需一种能在短时间内缓解地址短缺问题的方法，网络地址转换 (Network Address Translation, NAT) 技术被提了出来。其核心思想是让一个局域网中的每一个设备通过路由器共享一个公网 IP，局域网中的每一个设备被分配一个内网 IP（不参与 IPv4 的分配），在需要访问公网服务时，路由器记录「局域网设备的内网 IP + 端口」和其想要访问的「公网 IP 和端口」，作为代理转发流量。

NAT 技术不仅减缓了 IPv4 的消耗，也提升了内网设备的安全性，因为内网设备通过 NAT 访问公网服务是单向的。
