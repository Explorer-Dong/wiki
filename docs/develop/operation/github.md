...
使用 ssh 进行鉴权就很简单了。[创建密钥对](../operation/ssh.md#客户端生成密钥对) 后把公钥上传到 [GitHub](https://github.com/settings/keys)，然后本地 [配置 ssh config](../operation/ssh.md#ssh-config) 让对应的私钥指向 `github.com` 即可。
