...
文件共享的宗旨就是在尽可能减少存储开销的情况下获得更多访问文件的入口，技术实现上主要就是文件链接。

在类 Unix 操作系统上，链接的方式有如下两种：

- 硬链接，即拷贝文件的 `inode` 而非拷贝文件，即多个指针指向同一块硬盘；
- 软链接（符号链接），本质上是一个特殊的文件，保存的是一个字符串，记录目标文件/文件夹的路径。

下图给出了 [创建软硬链接](../../../develop/operation/linux/#创建链接-ln) 后的文件列表：
