...
- [**训练框架 (Training Framework)**](./training-framework/numpy.md)。包括科学计算与可视化库（NumPy、Pandas、Matplotlib）、深度学习框架（PyTorch、TensorFlow、JAX）、分布式训练（DDP、FSDP、ZeRO、Megatron-LM、DeepSpeed）、编译与优化（TorchInductor、Triton、XLA、TVM）、训练效率提升（混合精度训练、动态调度）等。负责将硬件资源转化为模型能力，是算力与算法结合的桥梁；
