...
首先需要 [安装 Git LFS 软件](https://docs.github.com/zh/repositories/working-with-files/managing-large-files/installing-git-large-file-storage)，然后在仓库中验证安装：
...
接着与 `.gitignore` 类似，将需要托管的大文件路径保存到 `.gitattributes` 文件中即可。例如想要托管 data 文件夹下的所有文件：

```bash
git lfs track data/*
```

就会在项目根目录自动生成一个 `.gitattributes` 文件，其中应当出现类似以下内容的文本：
