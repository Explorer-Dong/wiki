...
[GitHub Actions](https://docs.github.com/zh/actions) 是 GitHub 原生提供的 CI/CD 平台，可用于自动执行生成、测试和部署管道。开发者可以创建工作流，以便在推送更改到存储库时运行测试，或将合并的拉取请求部署到生产环境。笔者目前只接触了 GitHub Action，所以就以它为例展开讲解。
